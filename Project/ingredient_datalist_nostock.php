<?php
$page_name = 'ingredient_datalist_nostock';
$page_title = '缺貨商品列表';
require __DIR__ . "/ingredient_connect.php";
$page = isset($_GET['page']) ? intval($_GET['page']) : 1;
// $totalPage = $totalRows / $perPage;
$tst = $pdo->query('SELECT COUNT(*) FROM `ingredient` WHERE `ingredient`.`quantity` = 0 ');
...
if ($page < 1) {
    header("Location:ingredient_datalist_nostock.php?page=1");
    $page = 1;
}
if ($page > $totalPages) {
    header("Location:ingredient_datalist_nostock.php?page={$totalPages}");
    $page = $totalPages;
}
            ...
            <nav>
                <ul class="navbar-nav">
                    <li class="nav-item <?= $page_name == 'datalist' ? 'active' : '' ?> ">
                        <a class="nav-link" href="ingredient_datalist_nostock.php"><button type="button" class="btn btn-outline-warning" style="width:100px;">批量刪除</button></a>
                    </li>
                </ul>
            </nav>
                ...
                <ul class="pagination" style="margin:0;">
                    ...
                    for ($i = $p_start; $i < $p_end + 1; $i++) :
                        ...
                        <li class="page-item  <?= $i == $page ? 'active' : '' ?>">
                            <a class="page-link" style="margin:0 auto;" href="ingredient_datalist_nostock.php?page=<?php echo $i ?>"><?= $i ?></a>
                        </li>
                    <?php endfor; ?>
