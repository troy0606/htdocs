...
$page_name = 'appliance_list_edit';
$page_title = '編輯商品資料';

// 抓取item_sid的商品資訊 start

$item_sid = isset($_GET['item_sid']) ? intval($_GET['item_sid']) : 0;
if (empty($_GET['item_sid'])) {
...
$sql = "SELECT * FROM `appliance_list` WHERE `item_sid`=$item_sid";
$stmt = $pdo->query($sql);
$row = $stmt->fetch();
...
$sort_sid_now = $row['item_sort_sid'];

$sort_now_sql = "SELECT `appliance_list`.`item_sort_sid`,`item_sort`.* FROM `appliance_list` JOIN `item_sort` ON `appliance_list`.`item_sort_sid` = `item_sort`.`sort_sid` WHERE `appliance_list`.`item_sid` = $item_sid" ;
$sort_now_stmt = $pdo->query($sort_now_sql);
$sort_now_row = $sort_now_stmt->fetch();

$sort_parent_sid_now = $row['item_sort_sid'] == $sort_row['sort_sid'] ? $sort_row['sort_parent_sid'] : '';
...
<nav class="navbar navbar-expand-lg">
    <h3 style="color:#FFC107; padding: 0 50px 0 0;">編輯商品頁面</h3>
    <ul class="navbar-nav">
    ...
    <div style="margin-top:2rem;">
        ...
        <div class="card  text-white bg-dark" style="box-shadow: 0px 0px 80px #000000;">
            <div class="card-body">
                ...
                <!--  -->
                <input type="hidden" name="item_sid" value="<?= htmlentities($row['item_sid']) ?>">    
                
                <div class="form-group">
                        <!-- <label for="my_file">選擇上傳的圖檔</label> -->
                        <input type="file" class="form-control-file" id="item_image" name="item_image" style="display:none;"value="<?= htmlentities($row['item_image']) ?>" onchange="previewFile()">
                        <img id="item_image_preview" src="<?= 'uploads/' . $row['item_image'] ?>" height="200" alt="">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group d-flex align-items-center">
                        ...
                        <input type="text" class="form-control" style="width:300px; margin:0 20px 0 0;" value="<?= htmlentities($row['item_name']) ?>" id="item_name" name="item_name">
                        ...
                        <input type="text" class="form-control" style="width:300px; margin:0 20px 0 0;" value="<?= htmlentities($row['item_number']) ?>" id="item_number" name="item_number">
                    ...
                    <div class="form-group d-flex align-items-center">
                        ...
                        <input type="text" class="form-control" style="width: 300px; margin:0 20px 0 0;" value="<?= htmlentities($row['item_price']) ?>" id="item_price" name="item_price">
                        <small id="item_priceHelp" class="form-text"></small>
                        <label for="item_quantity" style="width:100px; margin:0 20px 0 0;">商品數量</label>
                        <input type="text" class="form-control" style="width: 300px; margin:0 20px 0 0;" value="<?= htmlentities($row['item_quantity']) ?>" id="item_quantity" name="item_quantity">
                        <small id="item_quantityHelp" class="form-text"></small>
                    </div>
                    <div class="form-group d-flex align-items-center">
                        <label for="item_status_sid" style="width:100px; margin:0 20px 0 0;">商品狀態</label>
                        <select class="form-control" style="width: 300px; margin:0 20px 0 0;" c id="item_status_sid" name="item_status_sid">
                            <?php foreach ($status_row as $v) : ?>
                                <option value="<?= $v['status_sid'] ?>"<?= $v['status_sid'] == $row['item_status_sid'] ? "selected" : "" ?>><?= $v['status_name'] ?></option>
                            <?php endforeach; ?>
                        </select>
                        <small id="item_status_sidHelp" class="form-text"></small>
                        <div class="form-check form-check-inline">
                            <input type="checkbox" class="form-check-input" id="item_hot_sell" name="item_hot_sell" value="<?= htmlentities($row['item_hot_sell']) ?>"<?= htmlentities($row['item_hot_sell']) ? 'checked' : '' ?>>
                            <label for="item_hot_sell" style="width:80px; margin:0;">熱銷</label>
                            <small id="item_hot_sellHelp" class="form-text"></small>

                            <input type="checkbox" class="form-check-input" id="item_new_listing" name="item_new_listing" value="<?= htmlentities($row['item_new_listing']) ?>"<?= htmlentities($row['item_new_listing']) ? 'checked' : '' ?>>
                            <label for="item_new_listing" style="width:80px; margin:0;">新上市</label>
                            <small id="item_new_listingHelp" class="form-text"></small>

                            <input type="checkbox" class="form-check-input" id="item_hit_month" name="item_hit_month" value="<?= htmlentities($row['item_hit_month']) ?>"<?= htmlentities($row['item_hit_month']) ? 'checked' : '' ?>>
                            <label for="item_hit_month" style="width:80px; margin:0;">本月主打</label>
                            <small id="item_hit_monthHelp" class="form-text"></small>

                            <input type="checkbox" class="form-check-input" id="item_recommend" name="item_recommend" value="<?= htmlentities($row['item_recommend']) ?>"<?= htmlentities($row['item_recommend']) ? 'checked' : '' ?>>
                    ...
                    <div class="form-group d-flex align-items-center">
                        ...
                        <select class="form-control" style="width: 300px; margin:0 20px 0 0;" id="item_place_sid" name="item_place_sid">
                            <?php foreach ($place_row as $v) : ?>
                                <option value="<?= $v['place_sid'] ?>"<?= $v['place_sid'] == $row['item_place_sid'] ? "selected" : "" ?>><?= $v['place_name'] ?></option>
                            <?php endforeach; ?>
                        </select>
                        ...
                        <select class="form-control" style="width: 300px; margin:0 20px 0 0;" id="item_brand_sid" name="item_brand_sid">
                            <?php foreach ($brand_row as $v) : ?>
                                <option value="<?= $v['brand_sid'] ?>"<?= $v['brand_sid'] == $row['item_brand_sid'] ? "selected" : "" ?>><?= $v['brand_name'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-group d-flex align-items-center">
                        ...
                        <input type="text" class="form-control"style="width: 200px; margin:0 20px 0 0;" value="<?= htmlentities($row['item_feature_1']) ?>" id="item_feature_1" name="item_feature_1">
                        ...
                        <input type="text" class="form-control"style="width: 200px; margin:0 20px 0 0;" value="<?= htmlentities($row['item_feature_2']) ?>" id="item_feature_2" name="item_feature_2">
                        ...
                        <input type="text" class="form-control"style="width: 200px; margin:0 20px 0 0;" value="<?= htmlentities($row['item_feature_3']) ?>" id="item_feature_3" name="item_feature_3">
                        <small id="item_feature_3Help" class="form-text"></small>
                    </div>
                    <div class="form-group">
                        <label for="item_format">規格</label>
                        <input type="text" class="form-control" value="<?= htmlentities($row['item_format']) ?>" id="item_format" name="item_format">
                        <small id="item_formatHelp" class="form-text"></small>
                    </div>
                    <div class="form-group">
                        <label for="item_detail">商品說明/介紹</label>
                        <input type="text" class="form-control" value="<?= htmlentities($row['item_detail']) ?>" id="item_detail" name="item_detail">
                        <small id="item_detailHelp" class="form-text"></small>
                    </div>

                    <button type="submit" class="btn btn-outline-warning" id="submit_btn">編輯</button>
                </form>
            </div>
        </div>
    </div>
