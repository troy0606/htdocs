<?php
$page_name = 'ingredient_datalist_offsale';
$page_title = '下架商品列表';
require __DIR__ . "/ingredient_connect.php";
$page = isset($_GET['page']) ? intval($_GET['page']) : 1;
// $totalPage = $totalRows / $perPage;
$tst = $pdo->query('SELECT COUNT(*) FROM `ingredient` WHERE `ingredient`.`sale` = 2 ');
...
if ($page < 1) {
    header("Location:ingredient_datalist_offsale.php?page=1");
    $page = 1;
}
if ($page > $totalPages) {
    header("Location:ingredient_datalist_offsale.php?page={$totalPages}");
    $page = $totalPages;
}
        ...
        <div class="collapse navbar-collapse justify-content-between align-items-center" id="navbarSupportedContent">
            <nav style="width: 250px;">
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item">
                        <button id='allOnSale' type="submit" class="btn btn-outline-warning" style="width:100px; margin-right:20px;" onclick="return confirm('確定上架嗎？')">批量上架</button>
                    </li>
                </ul>
            </nav>
                ...
                <ul class="pagination" style="margin:0;">
                    ...
                    for ($i = $p_start; $i < $p_end + 1; $i++) :
                        ...
                        <li class="page-item  <?= $i == $page ? 'active' : '' ?>">
                            <a class="page-link d-flex justify-content-center" style="width:55px; margin:0 auto; color: #15141A;" href="ingredient_datalist_offsale.php?<?= http_build_query($params) ?>"><?= $i ?></a>
                        </li>
...
<script>
    ...
    var goUp = document.getElementById('allOnSale');
    goUp.addEventListener('click', () => {
        form_submit.action = 'ingredient_allOnSale_api.php';
        form_submit.submit();
    })
</script>
